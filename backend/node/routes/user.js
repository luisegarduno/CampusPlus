    ...
    app.post('/user/login', (req, res) => {
        ...
        pool.getConnection(function (err, connection){
            ...
            res.status(400).send('Problem obtaining MySQL connection'); 
                ...
                connection.query('SELECT u.password, u.userID FROM `campusplus`.`user` u WHERE u.username = ?', [username], function (err, rows) {
                    if (err) { 
                        // if there's an error w/ the query, release the connection instance & log the error
                        connection.release()
                        res.status(400).json({
                        ...
                        bcrypt.compare(hashpass, rows[0].password, function (err, result) {
                            ...
                            if(result){
                                ...
                                connection.query('SELECT IF(EXISTS(SELECT * FROM `campusplus`.`user` u WHERE u.username = ? AND u.password = ?), (SELECT u.username AS result FROM `campusplus`.`user` u WHERE u.password = ?), 0) AS result', [username, rows[0].password, rows[0].password], function (err2, rows2) {
                                    connection.release()
                                    if (err2) {
